...
def run_single(run, force_sequential=False):
    ...
    try:
        ...
        err_msg = str(e)
        print(f"Error occurred during run: {err_msg}")
        is_oom = "OutOfMemoryError" in err_msg or "OOM" in err_msg or "out of memory" in err_msg.lower()
        
        # Explicitly clean up pipe reference and collect memory to avoid leaks
        if pipe is not None:
            del pipe
            pipe = None
        gc.collect()
...
for run_idx, run in enumerate(runs, 1):
    ...
    # Fallback if OOM and standard config was Model CPU Offload
    if res["status"] == "oom" and num_frames == 121:
    ...
    if res["status"] == "oom" or res["status"] == "failed":
        # Formulate a failure results dict for logging
        results[run_key] = {
            "input_id": input_id,
            "variant": variant,
            "num_frames": num_frames,
            ...
            "status": "failed",
            "error": res["error"],
            "offload_style": res["offload_style"],
            "fallback_to_sequential": num_frames == 121
        }
    else:
        results[run_key] = res

...
def run_tests():
    ...
    for vid in test_videos:
        ...
        assert assign_dp == assign_cv_relaxed, f"Mismatch for {vid}:\nDP:    {assign_dp}\nCV-eq: {assign_cv_relaxed}"
    print("PASS: CV-Align reduces to vanilla DP under relaxed constraints for all test videos")
    
    print("\n--- TEST 2: Constraint satisfaction ---")
    sim_matrix, scenes, video_dur = sim_matrices["review_7"]
    assign_cv_k2 = backend.cv_align_sequence(
        sim_matrix, scenes, video_dur,
        jump_penalty=0.01, reuse_bonus=0.01, backward_penalty=0.5,
        k_max=2, lam=0.1
    )
    ...
    max_consec = compute_max_consecutive(assign_cv_k2)
    ...
    print(f"CV-Align (K=2): {assign_cv_k2}")
    assert assign_dp_r7 != assign_cv_k2, "CV-Align did not change the assignment for review_7"
    print("PASS: CV-Align changed the looping assignment")
    
    print("\n--- TEST 4: Hyperparameter sweep ---")
    k_vals = [2, 3, 4]
    lam_vals = [0.0, 0.05, 0.1, 0.2]
    ...
    print("\n| Video | K_max | Lambda | Max Consec | Scene Diversity | VisCoher_strict | Assignment |")
    print("|-------|-------|--------|------------|-----------------|-----------------|------------|")
    for r in results:
        print(f"| {r['video']} | {r['k_max']} | {r['lam']} | {r['max_consec']} | {r['diversity']:.3f} | {r['viscoher_strict']:.3f} | {r['assignment']} |")

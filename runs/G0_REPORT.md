...
### 6. Canonical G0 Re-Evaluation (Multi-Sweep Validation)

Per the resumption order, we regenerated the sweeps using namespaced latents and deterministically scored them using `make_g0_tables.py`. This ensures a single source of truth for the thesis figures.

#### 6.1 Deterministic Full-Sweep Frontiers
...
#### 6.2 Aligned-DACA Summary

The table below reports advantage (`Δc̄ vs Frontier`) as the difference between the method's `c̄` and the interpolated baseline `c̄` at exactly matched `mc`.
...
*Note on prior versions*: The initial reported figures for geology legacy adaptive (0.255/0.853) were affected by a loop `break` bug that selected the minimum feasible $w$, mimicking the fixed-0.2 fallback. They are superseded by the correct Original DACA calculation (0.2638/0.7733) shown above.

#### 6.3 Gate Decision

**Criterion**: Original DACA `c̄` >= best-fixed `c̄` + 0.08 at matched `mean_concept` (±0.02).
**Evaluation**: All Original DACA configurations fail to achieve a positive advantage against the interpolated fixed-scale frontier. The Benefit-gated aligned variant achieves a minor +0.0024 to +0.0090 advantage, far below the threshold. 

**Verdict**: **FAIL**. The adaptive selection logic yields negative or statistically negligible advantage against the fixed-scale frontier across all tested regimes. Stage 1 (Alt-1) is blocked.

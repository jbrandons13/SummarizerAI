...
VIDEOS_CONFIG = [
    ("Heart",     "runs/heart/storyboard.json",                  "all",    "fullrun_results/data/V4_Heart_adaptive_anchor.csv",                              0.257, 0.856),
    ("Eye",       "data/intermediate/V12/phase4/storyboard.json", "all",    "fullrun_results/data/V12_Eye_adaptive_anchor.csv",                               0.284, 0.807),
    ("Sun",       "runs/sun/storyboard.json",                    "all",    "fullrun_results/data/V3_Sun_adaptive_anchor.csv",                                0.251, 0.799),
    ("Geology",   "runs/geology/storyboard.json",                "anchor", "fullrun_results/data/V1_Geology_adaptive_anchor.csv",                            0.339, 0.790),
    ("Ecology",   "runs/ecology/storyboard.json",                "anchor", "fullrun_results/data/V2_Ecology_adaptive_anchor.csv",                            0.310, 0.828),
    ("Neuron",    "data/intermediate/V10/phase4/storyboard.json", "anchor", "fullrun_results/data/V10_Neuron_adaptive_anchor.csv",                            0.252, 0.826),
    ("Volcano",   "data/intermediate/V11/phase4/storyboard.json", "anchor", "fullrun_results/data/V11_Volcano_adaptive_anchor.csv",                           0.283, 0.843),
    ("BlackHole", "data/intermediate/V6/phase4/storyboard.json",  "anchor", "fullrun_results/data/V6_BlackHole_adaptive_anchor.csv",                          0.287, 0.837),
    ("Hurricane", "data/intermediate/V13/phase4/storyboard.json", "anchor", "replacement_results_2026-06-15/3videos_results/V13_Hurricane_adaptive_anchor.csv", 0.322, 0.801),
    ("Rocket",    "data/intermediate/V15/phase4/storyboard.json", "anchor", "15videos_results/V15_Rocket_adaptive_anchor.csv",                                0.286, 0.804),
]
...
def main():
    device = "cuda"
    out_dir = "runs/external_baseline_3videos"  # reusing same outdir, will generate missing
    ...
    for vid_name, sb_path, selection, anchor_csv, dc, dp in VIDEOS_CONFIG:
        ...
        last_v = os.path.join(v_dir, f"shot_{n_shots-1:03d}.png")
        last_c = os.path.join(c_dir, f"shot_{n_shots-1:03d}.png")
        
        if os.path.exists(last_v) and os.path.exists(last_c):
            print(f"[{vid_name}] Images already exist, skipping generation...")
            continue
        ...
        if not os.path.exists(last_c):
            ...
            id_prompts = [ordered[i][1] for i in range(ID_LENGTH)]
            generators = [torch.Generator(device=device).manual_seed(ordered[i][2]) for i in range(ID_LENGTH)]
    ...
    for vid_name, sb_path, selection, anchor_csv, dc, dp in VIDEOS_CONFIG:
        ...
        fair_coords.append([vid_name, f"{mean_conc:.4f}", f"{mean_pres:.4f}", f"{dc:.4f}", f"{dp:.4f}"])
    ...
    with open("external_baseline_10videos_metrics.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    with open("external_baseline_10videos_summary.txt", "w") as f:
        f.write("StoryDiffusion Baseline (id_length=2) - 10 VIDEOS\n")
        f.write("\n".join(summary_lines))
    ...
    try:
        ...
        for ax, coord in zip(axes.ravel(), fair_coords):
            vid, bc, bp, dc, dp = coord
            bc, bp, dc, dp = float(bc), float(bp), float(dc), float(dp)
            ax.scatter([dc], [dp], marker="*", s=300, color="#d4ac0d", label="DACA")
            ax.scatter([bc], [bp], marker="o", s=140, color="#c0392b", label="Baseline")
            ax.set_title(vid)
            ax.grid(True, alpha=0.3)
            ax.legend(fontsize=8, loc="best")
        fig.suptitle("Fair plane: external baseline (StoryDiffusion) vs DACA — 10 videos", fontsize=14)
        fig.tight_layout()
        fig.savefig("external_10videos_fairplane.png", dpi=130)

...
FRAME_PATH = BASE_PATH / "concept_anchor_canonical_w02/images/shot_004.png"
COMFY_DIR = Path(os.path.expanduser("~/comfyui/ComfyUI"))
COMFY_INPUT_DIR = COMFY_DIR / "input"
COMFY_OUTPUT_DIR = COMFY_DIR / "output"
OUTPUT_PATH = BASE_PATH / "_smoke_test/shot_004_w02_living_still.webp"
...
def main():
    ...
    try:
        ...
        shutil.copy(FRAME_PATH, COMFY_INPUT_DIR / "smoke_w02_frame.png")
        ...
        with open(BASE_PATH / "storyboard.json") as f:
            ...
            shot_004 = next(s for s in storyboard["shots"] if s["shot_id"] == "shot_004")
            prompt_text = shot_004["visual_description"]
            
        workflow["5"]["inputs"]["image"] = "smoke_w02_frame.png"
        workflow["6"]["inputs"]["image"] = "smoke_w02_frame.png"
        workflow["9"]["inputs"]["text"] = prompt_text
        workflow["11"]["inputs"]["length"] = 49  # about 3 seconds
        workflow["14"]["inputs"]["filename_prefix"] = "smoke_test_shot_004_w02"
        ...
        if stats and "devices" in stats:
            for dev in stats["devices"]:
                if dev.get("type") == "cuda":
                    # Try to estimate peak VRAM from system stats if possible
                    # Or just report current VRAM
        ...
        # Don't kill ComfyUI process here just yet, or maybe do if we started it
        if comfy_proc:

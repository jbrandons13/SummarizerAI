...
vid = "V15"
vname = "River"
concept_text = "a colorful cartoon illustration of a river flowing through the landscape"
...
shots_desc = [
    "A small river spring bubbling up from a rocky mountain peak covered in bright white snow",
    "A river cascading down a steep cliff forming a massive roaring waterfall in a tropical jungle",
    "A winding river cutting deeply through a dry red rock desert canyon",
    "A wide calm river reflecting the city lights as it passes through a modern metropolis at night",
    "A frozen river completely covered in thick ice in a barren arctic tundra",
    "A shallow river flowing gently over colorful pebbles in an autumn forest with orange leaves",
    "A muddy brown river surging powerfully during a heavy rainstorm in a valley",
    "A crystal clear underground river flowing through a dark cave with glowing blue crystals",
    "A river winding its way through endless flat green agricultural farm fields",
    "A river passing under an ancient ruined stone bridge in a misty medieval village",
    "A river forming a wide delta with many branching streams meeting the ocean",
    "A river flowing calmly through a vibrant purple flower field in the spring",
    "A river choked with floating logs in a dense pine logging forest",
    "A river glowing with bioluminescence at twilight near a swamp",
    "A river passing through a dark volcanic landscape with black sand and smoke",
    "A river widening into a vast calm lake reflecting the starry night sky"
]
...
print(f"\n--- SWEEPING {vid} ---")
ret = run_cmd(f"PYTHONPATH=. python weight_sweep.py --config configs/default.yaml --storyboard {run_dir}/storyboard.json --reference gate_check/{vid}_{vname}_reference.png --shots {sids} --weights {w_str} --out {run_dir}/sweep")
if not ret:
    sys.exit(1)
